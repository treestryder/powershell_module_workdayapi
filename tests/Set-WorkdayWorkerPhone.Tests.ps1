...
'@ -f (Get-Date)
            $response = Set-WorkdayWorkerPhone -WorkerId 1 -WorkPhone 1234567890
        ...
        It 'Throws an exception when an invalid phone number is supplied.' {
            { Set-WorkdayWorkerPhone -WorkerId 1 -WorkPhone BadNumber} | Should Throw 'Unable to update Work phone number, invalid number: BadNumber'
        }
    }
}